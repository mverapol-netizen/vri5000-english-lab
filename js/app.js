...
function renderPractice(){
  ...
  document.getElementById('labOral').onclick=renderOralLab;
  document.getElementById('labAcademic').onclick=renderAcademicLab;
...
function renderProgress(){
  ...
  const weak=[...ranked].filter(x=>x.s.attempts>=3).sort((a,b)=>b.s.needScore-a.s.needScore).slice(0,3);
  ...
  const chunkHist=state.chunkHistory||{};
  const chunksStudied=Object.keys(chunkHist).length;
  const chunksDue=Object.values(chunkHist).filter(x=>x.reviewAt&&new Date(x.reviewAt).getTime()<=Date.now()).length;
  const listeningRuns=state.sessions.filter(x=>x.type==='listening');
  const listeningAvg=listeningRuns.length?Math.round(listeningRuns.slice(0,10).reduce((a,x)=>a+(x.coverage||0),0)/Math.min(10,listeningRuns.length)):0;
  const shadowRuns=state.sessions.filter(x=>x.type==='shadowing');
  const conversationRuns=state.sessions.filter(x=>x.type==='conversation');
  ...
  (weak.length?card('<div class="kicker">Recommended focus</div><h3>Highest current learning need</h3>'+weak.map(x=>'<div class="mini-row"><strong>'+esc(x.c.name)+'</strong><span style="float:right">need '+x.s.needScore+' · mastery '+x.s.mastery+'%</span></div>').join('')+'<p class="muted small">Need combines low mastery, controlled→transfer gap, response speed and uncertainty from your actual use.</p><button class="primary" id="weakMix">Practice these areas</button>','urgent-card'):'')+
  card('<div class="grid"><div class="metric"><strong>'+speakingRuns.length+'</strong><span>speaking audits</span></div><div class="metric"><strong>'+speakingAvg+'%</strong><span>recent speaking coverage</span></div><div class="metric"><strong>'+listeningAvg+'%</strong><span>recent listening coverage</span></div><div class="metric"><strong>'+chunksStudied+'</strong><span>chunks studied</span></div><div class="metric"><strong>'+chunksDue+'</strong><span>chunks due</span></div><div class="metric"><strong>'+conversationRuns.length+'</strong><span>conversation runs</span></div><div class="metric"><strong>'+shadowRuns.length+'</strong><span>shadowing runs</span></div><div class="metric"><strong>'+exams.length+'</strong><span>full rehearsals</span></div></div>')+
  '<div class="section-title">Mastery by concept</div>'+
  ranked.map(({c,s})=>card('<div class="row between"><strong>'+esc(c.name)+'</strong>'+pill(s.status)+'</div><div class="progressbar"><span style="width:'+s.mastery+'%"></span></div><div class="transfer-row"><span>Controlled</span><div class="progressbar"><span style="width:'+s.rates.controlled+'%"></span></div><strong>'+s.rates.controlled+'%</strong></div><div class="transfer-row"><span>Guided</span><div class="progressbar"><span style="width:'+s.rates.guided+'%"></span></div><strong>'+s.rates.guided+'%</strong></div><div class="transfer-row"><span>Transfer</span><div class="progressbar"><span style="width:'+s.rates.free+'%"></span></div><strong>'+s.rates.free+'%</strong></div><p class="muted small">'+(s.attempts||0)+' attempts · '+(s.counts?.free||0)+' transfer attempts · '+(s.avgResponseMs?Math.round(s.avgResponseMs/100)/10+' s avg':'no speed baseline')+' · '+s.uncertainty+'% unsure/guess</p>')).join('')+
...
function renderShadowingLab(){
  ...
  app.innerHTML=card('<div class="row between">'+pill(s.domain)+pill(s.focus,'gold')+'</div><h2>1. First listen · gist</h2><p class="muted">Listen without the transcript and write the main idea in one sentence.</p><button class="primary" id="shadowListen1">First listen</button><textarea class="input textarea" id="shadowGist" placeholder="Main idea…"></textarea>')+
  card('<h2>2. Second listen · details</h2><p class="muted">Listen again and note two or three specific details, words or phrases you recognized.</p><div class="grid"><button class="secondary" id="shadowListen2">Second listen</button><button class="secondary" id="shadowReveal">Compare transcript</button><button class="secondary" id="shadowNew">New passage</button></div><textarea class="input textarea" id="shadowDetails" placeholder="Details you heard…"></textarea><div id="shadowTranscript"></div>')+
  card('<h2>3. Shadowing</h2><p>After comparing the transcript, play the model again and speak almost simultaneously with it. Imitate rhythm, stress and chunking rather than individual words.</p><div class="big-timer" id="sTimer">40</div><div class="grid"><button class="primary" id="recordBtn">Record shadowing</button><button class="secondary" id="shadowModel">Play model</button></div><div id="audioBox"></div><div class="divider"></div><h3>Self-audit</h3><label class="check-line"><input type="checkbox" class="shadowCheck">I kept natural thought groups.</label><label class="check-line"><input type="checkbox" class="shadowCheck">I stressed the key content words.</label><label class="check-line"><input type="checkbox" class="shadowCheck">I stayed close to the model rhythm.</label><button class="secondary" id="shadowSave">Save full cycle</button>')+
  card('<h2>4. Reuse</h2><p>'+esc(s.reusePrompt)+'</p><div>'+s.chunks.map(x=>pill(x)).join('')+'</div><textarea class="input textarea" id="shadowReuse" placeholder="Write notes, then say it aloud…"></textarea>');
  ...
  document.getElementById('shadowSave').onclick=()=>{
    ...
    state.sessions.unshift({date:new Date().toISOString(),label:'Shadowing Lab',type:'shadowing',domain:s.domain,focus:s.focus,coverage,gist:document.getElementById('shadowGist').value.trim(),details:document.getElementById('shadowDetails').value.trim(),reuse:document.getElementById('shadowReuse').value.trim()});
