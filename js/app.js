...
function renderProgress(){
  ...
  (active.length?'<div class="section-title">Error Bank</div>'+active.slice(0,10).map(e=>card('<strong>'+esc(C[e.concept]?.name||e.concept)+'</strong><p>'+esc(e.prompt)+'</p><p class="small">Your answer: '+esc(e.userAnswer)+'<br>Correct: '+esc(e.correctAnswer)+'</p><p class="muted small">'+(e.occurrences||1)+' occurrence(s)</p><button class="secondary repairBtn" data-c="'+e.concept+'">Repair this area</button>')).join(''):'');
